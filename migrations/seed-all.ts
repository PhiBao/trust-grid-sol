...
const secretKey = Uint8Array.from(JSON.parse(fs.readFileSync((process.env.ANCHOR_WALLET || `${os.homedir()}/.config/solana/id.json`), "utf-8")));
...
async function getOrCreateATA(mint: PublicKey, owner: PublicKey): Promise<PublicKey> {
  const ata = await getAssociatedTokenAddress(mint, owner, false);
  const info = await connection.getAccountInfo(ata);
  if (!info) {
    const ix = createAssociatedTokenAccountInstruction(wallet.publicKey, ata, owner, mint);
...
async function registerAgent(name: string, uri: string, meta: [string, string][], id: number): Promise<PublicKey> {
  const agentPda = getPda([Buffer.from("agent"), wallet.publicKey.toBuffer(), toU64LE(id)]);
  ...
  const data = Buffer.concat([D.register, writeString(uri), metaLen, metaEncoded]);
...
async function createTask(agentId: number, amount: number, deadline: number, uri: string, taskNum: number) {
  ...
  const sig = await sendIX([
    ...
    { pubkey: SystemProgram.programId, isSigner: false, isWritable: false },
    { pubkey: (await connection.getMinimumBalanceForRentExemption(165)), isSigner: false, isWritable: false } as any, // rent sysvar placeholder
  ], data);
...
async function giveFeedback(agentId: number, value: number, tag: string, feedbackIdx: number) {
  const repPda = getPda([Buffer.from("reputation"), toU64LE(agentId)]);
  const [feedbackPda] = PublicKey.findProgramAddressSync(
    [Buffer.from("feedback"), toU64LE(agentId), wallet.publicKey.toBuffer(), toU64LE(feedbackIdx)],
    PROGRAM_ID
  );
  ...
  const agentPda = getPda([Buffer.from("agent"), wallet.publicKey.toBuffer(), toU64LE(agentId)]);
...
async function main() {
  ...
  console.log("\n🤖 Registering agents...");
  const agents = [
    { name: "Nemesis Auditor", uri: "https://trustgrid.xyz/agents/nemesis.json", meta: [["name","Nemesis Auditor"],["skill","smart_contract_audit"],["category","security"],["framework","rust"],["price","1.0 USDC"],["endpoint","https://nemesis.trustgrid.xyz/mcp"],["description","Smart contract security auditor using dual-pass methodology."]] },
    { name: "Alpha Trader", uri: "https://trustgrid.xyz/agents/alpha.json", meta: [["name","Alpha Trader"],["skill","defi_trading"],["category","trading"],["framework","python"],["price","0.5 USDC"],["endpoint","https://alpha.trustgrid.xyz/mcp"],["description","DeFi trading agent with MEV protection and real-time signals."]] },
    { name: "Data Oracle", uri: "https://trustgrid.xyz/agents/oracle.json", meta: [["name","Data Oracle"],["skill","data_aggregation"],["category","data"],["framework","typescript"],["price","0.2 USDC"],["endpoint","https://oracle.trustgrid.xyz/mcp"],["description","Real-time data aggregation and cross-chain verification."]] },
    { name: "Compliance Guard", uri: "https://trustgrid.xyz/agents/compliance.json", meta: [["name","Compliance Guard"],["skill","compliance_check"],["category","compliance"],["framework","rust"],["price","2.0 USDC"],["endpoint","https://compliance.trustgrid.xyz/mcp"],["description","ZKID-verified compliance checking for institutional agents."]] },
    { name: "Yield Farmer", uri: "https://trustgrid.xyz/agents/yield.json", meta: [["name","Yield Farmer"],["skill","yield_optimization"],["category","defi"],["framework","solidity"],["price","0.8 USDC"],["endpoint","https://yield.trustgrid.xyz/mcp"],["description","Automated yield farming across Solana DeFi protocols."]] },
    { name: "MEV Sentinel", uri: "https://trustgrid.xyz/agents/mev.json", meta: [["name","MEV Sentinel"],["skill","mev_protection"],["category","security"],["framework","rust"],["price","1.5 USDC"],["endpoint","https://mev.trustgrid.xyz/mcp"],["description","MEV extraction and sandwich attack protection."]] },
  ];
  ...
  for (const f of feedbacks) {
    try {
      ...
      console.log(`   ⚠️  Feedback ${f.agent}/${f.idx} skipped: ${e.toString().slice(0, 60)}`);
    }
  }
