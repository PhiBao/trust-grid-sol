...
const MPL_AGENT_REGISTRY = new PublicKey(
  "Ag1nwob8UsvqeX2Q1eDSTG4HQ6s6Wz6hQJdA6t6X1z9"
);
...
const DEMO_METAPLEX_AGENTS: MetaplexAgent[] = [
  {
    ...
    name: "Plexpert",
    description:
      "An informational agent providing help related to Metaplex protocols and tools.",
    services: [
      { name: "web", endpoint: "https://metaplex.com/agent/7xKX..." },
      {
        name: "MCP",
        endpoint: "https://metaplex.com/agent/7xKX.../mcp",
        version: "2025-06-18",
      },
    ],
  ...
  },
  {
    ...
    name: "MetaTrader",
    description:
      "Metaplex-registered DeFi trading agent with NFT-backed identity.",
    services: [
      { name: "trading", endpoint: "https://metatrader.agent/mcp" },
      {
        name: "A2A",
        endpoint: "https://metatrader.agent/a2a",
        version: "0.3.0",
      },
    ],
    ...
    registrations: [
      {
        agentId: "8yLYtg3DX98d98TYJSDpbD6jBkheTqB94TZRuJosgBsV",
        agentRegistry: "solana:101:metaplex",
      },
    ],
  },
...
export async function fetchMetaplexAgents(): Promise<OnChainAgent[]> {
  ...
  return DEMO_METAPLEX_AGENTS.map((mpl, idx) =>
    convertMetaplexAgent(mpl, 1000 + idx)
  );
}
 ...
 */
export function convertMetaplexAgent(
  mpl: MetaplexAgent,
  id: number
): OnChainAgent {
  const category = mpl.services.find((s) => s.name === "MCP")
    ? "mcp"
    : mpl.services.find((s) => s.name === "trading")
    ? "trading"
    : mpl.services.find((s) => s.name === "security")
    ? "security"
    : "general";

  const endpoint =
    mpl.services.find((s) => s.name === "MCP")?.endpoint ||
