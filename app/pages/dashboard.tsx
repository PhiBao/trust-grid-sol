...
export default function DashboardPage() {
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "register">(
    "overview"
  );
  ...
  const myTasks = publicKey
    ? tasks.filter((t) => t.client === publicKey.toBase58())
    : [];
  const myAgents = publicKey
    ? agents.filter((a) => a.authority === publicKey.toBase58())
    : [];
  ...
  const handleRegister = async () => {
    ...
    if (!regName.trim() || !regUri.trim()) {
      showToast("Name and URI are required");
      return;
    }
    setRegLoading(true);
    try {
          ...
          {activeTab === "overview" && (
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <StatCard
                  label="SOL Balance"
                  value={connected ? `${solBalance.toFixed(3)} SOL` : "—"}
                />
                <StatCard
                  label="USDC Balance"
                  value={connected ? `${usdcBalance.toFixed(2)} USDC` : "—"}
                />
                ...
                {myTasks.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {myTasks.map((t) => (
                      <div
                        key={t.taskId}
                        className="bg-parchment rounded-utility p-4 flex items-center justify-between"
                      >
                        <div>
                          <p className="text-caption-strong text-ink">
                            Task #{t.taskId}
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Nemesis Auditor"
                  />
                </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <select
                        ...
                        className="w-full bg-parchment rounded-utility px-4 py-2.5 text-body-apple text-ink outline-none focus:ring-2 focus:ring-action-blue/30 appearance-none cursor-pointer"
                      >
                        {[
                          "security",
                          "trading",
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-parchment rounded-utility px-4 py-2.5 text-body-apple text-ink outline-none focus:ring-2 focus:ring-action-blue/30"
                    placeholder="https://your-agent.trustgrid.xyz/mcp"
                  />
                </div>
                <div>
                  <label className="text-fine text-ink/40 block mb-1.5">
                  ...
                  <input
                    ...
                    className="w-full bg-parchment rounded-utility px-4 py-2.5 text-body-apple text-ink outline-none focus:ring-2 focus:ring-action-blue/30"
                    placeholder="https://trustgrid.xyz/agents/my-agent.json"
                  />
                </div>
                <div>
                  <label className="text-fine text-ink/40 block mb-1.5">
                  ...
                  <textarea
                    ...
                    className="w-full bg-parchment rounded-utility px-4 py-2.5 text-body-apple text-ink outline-none focus:ring-2 focus:ring-action-blue/30 resize-none"
                    rows={3}
