...
export default function AgentDetailPage() {
  ...
  const handleGiveFeedback = async () => {
    ...
    try {
      ...
      const [rep, fbList] = await Promise.all([fetchReputation(agent.agentId), fetchFeedbacksForAgent(agent.agentId)]);
  ...
  return (
    <Layout>
      {toast && (
        <div className="fixed top-20 left-1/2 -translate-x-1/2 z-[60] bg-ink text-white text-sm px-5 py-3 rounded-pill shadow-product animate-bounce max-w-md">
          <div className="flex items-center space-x-3">
            ...
            {toast.signature && (
              <a href={getTxUrl(toast.signature)} target="_blank" rel="noopener noreferrer" className="text-action-blue hover:underline whitespace-nowrap" onClick={(e) => e.stopPropagation()}>View Tx →</a>
            )}
      ...
      <main className="max-w-grid mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <button onClick={() => router.push('/')} className="text-caption-apple text-action-blue hover:underline mb-8 inline-block">← Back to Marketplace</button>
          ...
          <>
            ...
            <div className="flex flex-col md:flex-row md:items-start md:space-x-6 mb-10">
              ...
              <div className="flex-1">
                ...
                <span className="inline-block text-micro text-action-blue bg-action-blue/5 px-3 py-1 rounded-utility capitalize mb-3">{cat}</span>
                <p className="text-body-apple text-ink/60 max-w-2xl">{agent.metadata["description"] || getAgentSkill(agent)}</p>
                <div className="mt-3 flex items-center space-x-4">
                  <a href={getAccountUrl(agent.pda)} target="_blank" rel="noopener noreferrer" className="text-caption-apple text-action-blue hover:underline">View Agent PDA →</a>
                  <a href={getAccountUrl(agent.authority)} target="_blank" rel="noopener noreferrer" className="text-caption-apple text-action-blue hover:underline">View Authority →</a>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                <div className="bg-parchment rounded-card p-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    {reputation && (
                      <a href={getAccountUrl(agent.pda.replace(/...$/, '') + 'reputation' /* fallback */)} target="_blank" rel="noopener noreferrer" className="text-caption-apple text-action-blue hover:underline">View Reputation PDA →</a>
                    )}
                  </div>
                  {reputation && reputation.feedbackCount > 0 ? (
                    <div className="flex items-center space-x-1 mb-4">
                      {[1,2,3,4,5].map((s) => <span key={s} className={`text-xl ${s <= Math.round(reputation.averageScore/100) ? 'text-yellow-500' : 'text-ink/20'}`}>★</span>)}
                  ...
                  {feedbacks.length > 0 && (
                    <div className="mt-4 border-t border-hairline pt-4">
                      ...
                      <div className="space-y-3 max-h-64 overflow-y-auto pr-1">
                        {feedbacks.map((fb, i) => (
                          <a key={i} href={getAccountUrl(fb.pda)} target="_blank" rel="noopener noreferrer" className="block bg-white rounded-utility p-3 flex items-start space-x-3 hover:border-action-blue/30 border border-transparent transition-colors">
                            <div className="flex items-center space-x-0.5 flex-shrink-0">
                              {[1,2,3,4,5].map((s) => <span key={s} className={`text-xs ${s <= fb.value ? 'text-yellow-500' : 'text-ink/20'}`}>★</span>)}
                            </div>
                            ...
                            <span className="text-fine text-action-blue flex-shrink-0">View →</span>
                          </a>
                        ))}
                      </div>
                    </div>
                ...
                <div className="bg-parchment rounded-card p-6">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center space-x-3">
                      <a href={getAccountUrl(agent.pda)} target="_blank" rel="noopener noreferrer" className="text-caption-apple text-action-blue hover:underline">View on Explorer →</a>
              ...
              <div className="space-y-6">
                <div className="bg-white rounded-card border border-hairline p-6 sticky top-20">
                  ...
                  <p className="text-body-apple text-ink/60 mb-6">Create a task with USDC escrow. Funds are held trustlessly until completion.</p>
                  {connected && (
                    <div className="space-y-2 mb-4">
                      <div className="bg-parchment rounded-utility p-3 flex items-center justify-between"><span className="text-fine text-ink/40">Your SOL</span><span className="text-caption-strong text-ink">{solBalance.toFixed(3)} SOL</span></div>
                      <div className="bg-parchment rounded-utility p-3 flex items-center justify-between"><span className="text-fine text-ink/40">Your USDC</span><span className="text-caption-strong text-ink">{usdcBalance.toFixed(2)} USDC</span></div>
                    </div>
                  )}
                  <button onClick={() => setHireOpen(true)} className="apple-pill w-full justify-center mb-3">Hire for {getAgentPrice(agent)}</button>
                ...
                <div className="bg-white rounded-card border border-hairline p-6">
                  ...
                  <div className="flex items-center space-x-2 mb-4">
                    {[1,2,3,4,5].map((s) => <button key={s} onClick={() => setFeedbackValue(s)} className={`text-2xl transition-colors ${s <= feedbackValue ? 'text-yellow-500' : 'text-ink/20 hover:text-ink/40'}`}>★</button>)}
                    <span className="text-fine text-ink/50 ml-2">{feedbackValue}/5</span>
                  </div>
                  <input type="text" value={feedbackTag} onChange={(e) => setFeedbackTag(e.target.value)} placeholder="Tag (e.g. excellent, fast)" className="w-full bg-parchment rounded-utility px-4 py-2.5 text-body-apple text-ink outline-none focus:ring-2 focus:ring-action-blue/30 mb-3" maxLength={50} />
                  <button onClick={handleGiveFeedback} disabled={feedbackLoading || !connected} className="apple-pill-ghost w-full justify-center disabled:opacity-50">
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setHireOpen(false)}>
          <div className="bg-white rounded-card max-w-md w-full p-6 md:p-8 shadow-product" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-start justify-between mb-6">
              <div><h3 className="text-section text-ink">Create Task</h3><p className="text-caption-apple text-ink/50 mt-1">Hire {name}</p></div>
            ...
            <div className="space-y-4 mb-6">
              <div><label className="text-fine text-ink/40 block mb-1.5">Amount (USDC)</label><input type="number" value={taskAmount} onChange={(e) => setTaskAmount(e.target.value)} className="w-full bg-parchment rounded-utility px-4 py-2.5 text-body-apple text-ink outline-none focus:ring-2 focus:ring-action-blue/30" placeholder="1.0" min="0.1" step="0.1" /></div>
              <div><label className="text-fine text-ink/40 block mb-1.5">Task Description URI</label><input type="text" value={taskUri} onChange={(e) => setTaskUri(e.target.value)} className="w-full bg-parchment rounded-utility px-4 py-2.5 text-body-apple text-ink outline-none focus:ring-2 focus:ring-action-blue/30" placeholder="https://..." /></div>
            </div>
            <div className="bg-parchment rounded-utility p-3 mb-6">
              <div className="flex items-center justify-between text-caption-apple text-ink/60"><span>Service Fee (1%)</span><span>{(parseFloat(taskAmount || "0") * 0.01).toFixed(3)} USDC</span></div>
              <div className="flex items-center justify-between text-body-strong text-ink mt-1"><span>Total</span><span>{(parseFloat(taskAmount || "0") * 1.01).toFixed(3)} USDC</span></div>
            </div>
